...
public void test() {
       EmailContentReq emailContentReq = new EmailContentReq();
        emailContentReq.setTenantId(123L);
        emailContentReq.setContent("群发id测试");
        emailContentReq.setSenderName("test");
        emailContentReq.setSubject("群发id测试");
        emailContentReq.setAppId(0L);


        ReceiveObj receiveObj1= new ReceiveObj();
        receiveObj1.setReceiver("1");
        receiveObj1.setReceiverType(AddressType.user);

        ReceiveObj receiveObj2= new ReceiveObj();
        receiveObj2.setReceiver("1");
        receiveObj2.setReceiverType(AddressType.user);

        List<ReceiveObj> receiverList = Lists.newArrayList(receiveObj1,receiveObj2);


        emailContentReq.setReceiverList(receiverList);
        emailContentClient.send(emailContentReq);
}



```
#### 3.client-api升级 release notes
* 1.0-SNAPSHOT 初始
* 1.0.1 支持收件人用户id批量发送
